...
@REM ===== Handle files in CustomLVL\Sound\
@set SOURCE_SUBDIR=CustomLVL\Sound
@set SOURCE_DIR=
@if not %MUNGE_OVERRIDE_DIR%x==x (
	@for %%a in (%MUNGE_OVERRIDE_DIR%) do @set SOURCE_DIR=%SOURCE_DIR% %MUNGE_ROOT_DIR%\%%a\%SOURCE_SUBDIR%
)
...
levelpack -inputfile %INPUT_FILE% -common Common\MUNGED\%MUNGE_PLATFORM%\core.files Common\MUNGED\%MUNGE_PLATFORM%\common.files %MUNGE_ARGS% -sourcedir %SOURCE_DIR% -inputdir %MUNGE_DIR% -outputdir %OUTPUT_DIR% 2>>%MUNGE_LOG%
